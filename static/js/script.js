...
// --- AUDIO SYSTEM ---
const sounds = {
  click: new Audio("/static/audio/click.wav"),
  bull: new Audio("/static/audio/bull.wav"),
  miss: new Audio("/static/audio/miss.wav"),
  win: new Audio("/static/audio/game-win.mp3"),
  lose: new Audio("/static/audio/game-lose.mp3"),
};
    ...
    .then((data) => {
      ...
      if (data.valid) {
        ...
        if (data.game_over) {
          ...
          if (
            currentRow >= currentRenderedRows - 2 &&
            currentRenderedRows < maxTries
          ) {
...
function endGame(won, code) {
  ...
  }, 1500); // Give the sound effect slightly more time to play before redirect
}
...
function getHint() {
  ...
  fetch("/api/hint", { method: "POST" })
    ...
    .then((data) => {
      if (data.number)
        alert(`💡 HINT: Position ${data.index + 1} is ${data.number}`);
...
function showStats() {
  ...
  document.getElementById("stat-winpct").innerText =
